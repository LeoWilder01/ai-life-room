...
const LAND_RECTS: Array<[number, number, number, number]> = [
  [-170, -52, 24, 75], [-170, -130, 54, 72], [-58, -18, 60, 84],
  [-24, -13, 63, 67], [-82, -34, -56, 12], [-9, 4, 36, 44],
  [-5, 8, 43, 52], [-8, 2, 50, 59], [4, 32, 56, 72],
  [8, 32, 40, 56], [7, 18, 37, 47], [18, 28, 36, 46],
  [26, 45, 36, 43], [-17, 51, -35, 37], [32, 60, 12, 30],
  [44, 75, 25, 40], [43, 50, -26, -12], [60, 93, 6, 36],
  [28, 65, 50, 75], [60, 180, 50, 78], [50, 90, 36, 56],
  [73, 135, 18, 53], [126, 130, 34, 42], [129, 146, 30, 46],
  [92, 110, 0, 28], [95, 110, -6, 8], [105, 115, -9, -6],
  [108, 119, -5, 7], [118, 127, 4, 20], [131, 148, -10, 0],
  [113, 154, -39, -11], [166, 178, -47, -34],
];

function isLandPoint(lon: number, lat: number): boolean {
  return LAND_RECTS.some(([lo, hi, la, lb]) => lon >= lo && lon <= hi && lat >= la && lat <= lb);
}
...
const LAND_GRID: boolean[][] = (() => {
  ...
  for (let r = 0; r < GRID_H; r++) {
    ...
    for (let c = 0; c < GRID_W; c++)
      g[r][c] = isLandPoint((c + 0.5) * (360 / GRID_W) - 180, 90 - (r + 0.5) * (180 / GRID_H));
  }
  return g;
})();
...
function latLonToGrid(lat: number, lon: number) {
  return {
    col: Math.max(0, Math.min(GRID_W - 2, Math.floor(((lon + 180) / 360) * GRID_W))),
    row: Math.max(0, Math.min(GRID_H - 2, Math.floor(((90 - lat) / 180) * GRID_H))),
  };
}
  ...
  while (queue.length) {
    ...
    outer: for (let dr = 0; dr < 2; dr++)
      for (let dc = 0; dc < 2; dc++)
        if (occupied.has(`${col + dc},${row + dr}`)) { free = false; break outer; }
...
function findFreeTrailCell(
  ...
  const tc = cc(targetCol), tr = cr(targetRow);
  ...
  const expand = (c: number, r: number, level: number) => {
    for (let dc = -1; dc <= 1; dc++) {
      for (let dr = -1; dr <= 1; dr++) {
        ...
        const nc = cc(c + dc), nr = cr(r + dr);
        const key = `${nc},${nr}`;
        if (!visited.has(key)) { visited.add(key); queue.push([nc, nr, level]); }
      }
    }
  };
...
interface Agent { _id: string; name: string; description: string; claimStatus: string; hasPersona: boolean; lastActive?: string; }
interface Persona { displayName: string; birthPlace: { city: string; country: string }; birthDate: string; }
interface Intersection { _id: string; initiatingAgent: string; otherAgent: string; }
...
export default function WorldMap({ agentData, allLifeDays, intersections }: Props) {
  ...
  const lifeDaysByAgent = useMemo(() => {
    ...
    for (const days of map.values())
      days.sort((a, b) => new Date(b.fictionalDate).getTime() - new Date(a.fictionalDate).getTime());
    return map;
  }, [allLifeDays]);
  ...
  const { agentsOnMap, agentsNeedingLLM } = useMemo(() => {
    ...
    const needLLM: { name: string; displayName: string; city: string; country: string; agentIdx: number }[] = [];
    ...
    agentData.forEach((d, i) => {
      ...
      for (const day of days) { coords = getLifeDayCoords(day); if (coords) break; }

      if (!coords) {
        needLLM.push({ name: d.agent.name, displayName: d.persona?.displayName || d.agent.name, city: days[0].location.city, country: days[0].location.country, agentIdx: i });
        return;
      }
      const { col, row } = latLonToGrid(coords.lat, coords.lon);
      const cell = findFreeCell(col, row, occupied);
      if (!cell) return;
      for (let dr = 0; dr < 2; dr++) for (let dc = 0; dc < 2; dc++) occupied.add(`${cell.col + dc},${cell.row + dr}`);
      placed.push({ name: d.agent.name, displayName: d.persona?.displayName || d.agent.name, col: cell.col, row: cell.row, color: AGENT_COLORS[i % AGENT_COLORS.length], latestLifeDay: days[0], agentIdx: i });
    });
  ...
  useEffect(() => {
    ...
    async function resolve() {
      for (const { name, displayName, city, country, agentIdx } of agentsNeedingLLM) {
        const key = `${name}::${city}::${country}`;
        if (llmAttempted.current.has(key)) continue;
        llmAttempted.current.add(key);
        try {
          const res = await fetch("/api/geocode", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ city, country }) });
          const data = await res.json();
          if (typeof data.lat === "number" && typeof data.lon === "number") {
            const { col, row } = latLonToGrid(data.lat, data.lon);
            const days = lifeDaysByAgent.get(name) || [];
            if (!days.length) continue;
            setLlmAgents(prev => prev.some(a => a.name === name) ? prev : [...prev, { name, displayName, col, row, color: AGENT_COLORS[agentIdx % AGENT_COLORS.length], latestLifeDay: days[0], agentIdx }]);
          }
        } catch { /* skip */ }
      }
    }
  ...
  const allAgentsOnMap = useMemo(
    () => [...agentsOnMap, ...llmAgents.filter(a => !agentsOnMap.some(b => b.name === a.name))],
  ...
  const agentTrails = useMemo((): Map<string, TrailPoint[]> => {
    ...
    for (const agent of allAgentsOnMap) {
      ...
      days.forEach((day, dayIdx) => {
        ...
        if (coords) {
          ({ col: targetCol, row: targetRow } = latLonToGrid(coords.lat, coords.lon));
        } else {
          const seed = agent.name.length * 31 + day.roundNumber * 7;
          targetCol = Math.max(0, Math.min(GRID_W - 1, agent.col + seededInt(seed, -2, 2)));
          targetRow = Math.max(0, Math.min(GRID_H - 1, agent.row + seededInt(seed + 1, -2, 2)));
        }

        // Find free land cell near target (random walk → BFS)
        const baseSeed = (agent.name.charCodeAt(0) || 0) * 37 + day.roundNumber * 113 + dayIdx * 59;
        const cell = findFreeTrailCell(targetCol, targetRow, trailOccupied, baseSeed);
    ...
    >
        ...
        return trail.flatMap((point, idx) => {
          ...
          return ([-1, 0, 1] as const).map(copy => (
            <div
              ...
              onMouseEnter={e => {
                setHoveredItem({ type: "trail", agentName: agent.name, day: point.day });
                updatePopupPos(e);
              }}
              onMouseLeave={() => { setHoveredItem(null); setPopupPos(null); }}
            ...
            >
              <div
                style={{
                  ...
                  boxShadow: isHighlighted ? `0 0 6px 2px ${agent.color}88` : "none",
        ...
        return ([-1, 0, 1] as const).map(copy => (
          ...
          >
            {isConnected && !isHovered && (
              <div className="absolute rounded-full animate-pulse" style={{ width: avatarSize + 8, height: avatarSize + 8, background: "rgba(255,121,198,0.3)", top: "50%", left: "50%", transform: "translate(-50%,-50%)" }} />
            )}
      ...
      {hoveredItem && popupPos && (() => {
        if (hoveredItem.type === "agent") {
          ...
          const trailDays = (lifeDaysByAgent.get(agent.name) || []).slice(0, MAX_TRAIL);
          return <AgentPopup agent={agent} trailDays={trailDays} pos={popupPos} containerRef={containerRef} />;
        } else {
          const agent = agentPosMap.get(hoveredItem.agentName);
          return <TrailDayPopup day={hoveredItem.day} agentColor={agent?.color ?? "#aaa"} agentName={agent?.displayName ?? hoveredItem.agentName} pos={popupPos} containerRef={containerRef} />;
        }
      })()}
      ...
      {allAgentsOnMap.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ zIndex: 40 }}>
          <p className="text-gray-400 font-mono text-sm">No agents on the map yet</p>
        </div>
      )}
...
function AgentPopup({
  agent, trailDays, pos, containerRef,
}: {
  agent: AgentOnMap;
  trailDays: LifeDay[];
  ...
  return (
    <div
      ...
      style={{ left: px, top: py, width: POPUP_W, zIndex: 60, fontFamily: "'VT323', monospace" }}
    >
      {/* Agent header */}
      <div className="flex items-center gap-2 mb-3">
        ...
        <img
          ...
          alt={agent.displayName}
          width={36} height={36}
          style={{ width: 36, height: 36, borderRadius: "50%", border: `2px solid ${agent.color}`, background: "#f0f0f0", flexShrink: 0 }}
        />
        <div>
          <div style={{ color: agent.color, fontSize: 22, lineHeight: 1 }}>{agent.displayName}</div>
          <div style={{ color: "#8892a4", fontSize: 16, lineHeight: 1 }}>@{agent.name}</div>
          <div style={{ color: "#6a7a8e", fontSize: 14, lineHeight: 1 }}>
            {agent.latestLifeDay.location.city}, {agent.latestLifeDay.location.country}
          </div>
        </div>
      </div>
        ...
        <>
          ...
          {trailDays.map((day, i) => {
            const proxyPhoto = day.photo.originalUrl
              ...
              : null;
            return (
              <div key={day._id || i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "flex-start" }}>
                {/* Photo thumbnail */}
                <div style={{ width: 52, height: 40, flexShrink: 0, borderRadius: 2, overflow: "hidden", background: "#1a2a3a", border: `1px solid ${agent.color}44` }}>
                  {proxyPhoto && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={proxyPhoto} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  )}
                </div>
                {/* Text */}
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ color: agent.color, fontSize: 15, lineHeight: 1 }}>
                    Day {day.roundNumber} · Age {day.fictionalAge}
                  </div>
                  <div style={{ color: "#6a7a8e", fontSize: 13, lineHeight: 1 }}>
                    {day.location.city} · {fmtDate(day.fictionalDate)}
                  </div>
                  <div style={{
                    color: "#a8b2c4", fontSize: 14, lineHeight: 1.2, marginTop: 2,
...
function TrailDayPopup({
  day, agentColor, agentName, pos, containerRef,
}: {
  day: LifeDay;
  agentColor: string;
  agentName: string;
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: 6 }}>
        ...
        <div style={{ color: "#6a7a8e", fontSize: 14 }}>{fmtDate(day.fictionalDate)}</div>
      </div>

      {/* Photo */}
      {proxyPhoto && (
        <div style={{ width: "100%", height: 110, borderRadius: 3, overflow: "hidden", marginBottom: 6, border: `1px solid ${agentColor}44` }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={proxyPhoto} alt={day.photo.caption} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        </div>
      )}
      ...
      {/* Thought bubble */}
      <div style={{
        color: "#c8d8e8", fontSize: 16, lineHeight: 1.3,
        overflow: "hidden", display: "-webkit-box",
